...
#define linXCalibration 0.004305
#define linYCalibration -0.002111
#define linZCalibration 0.104113
...
float newGx, newGy, newGz;

float new_lin_ax, new_lin_ay, new_lin_az;
...
void accelerometerData() {
  ...
  // Confirmar valores usando filtro
  filter.updateIMU(newGx, newGy, newGz, newAx, newAy, newAz);
  float q0 = filter.q0;
  float q1 = filter.q1;
  float q2 = filter.q2;
  float q3 = filter.q3;
  ...
  velocityX += ((new_lin_ax + prev_lin_ax) * 0.5 * deltat) * 9.81;

  // Y calculus
  if(new_lin_ay > -0.001073	&& new_lin_ay < -0.000442){
    new_lin_ay = 0.00;
    velocityY = 0.00;
  }
  velocityY += ((new_lin_ay + prev_lin_ay) * 0.5 * deltat) * 9.81;
  
  // Z calculus
  if(new_lin_az > 0.003117 && new_lin_az < 0.008144){
    new_lin_az = 0.00;
    velocityZ = 0.00;
  }
  velocityZ += ((new_lin_az + prev_lin_az) * 0.5 * deltat) * 9.81;
...
void loop() {
    if (read) {
      ...
      Serial.print(ax, 4);
      Serial.print(",");
      Serial.print(newAx, 4);
      Serial.print(",");
      Serial.print(lin_ax, 4);
      Serial.print(",");
      Serial.print(new_lin_ax, 4);
      Serial.println();
